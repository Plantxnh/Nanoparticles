...
def draw(y_test,y_pred,score,title,colors):
    ...
    plt.text(1, 4, 'RMSE:{}\nR^2:{}'.format(round(math.sqrt(metrics.mean_squared_error(y_test, y_pred)), 2), round(score, 2)), family='Arial', fontsize=15)
    #添加点，alpha为点的透明度
...
if __name__=="__main__":
    ...
    x,y=data("Shannon.xlsx",21)
    #加载模型，修改名称加载不同的模型
    model = joblib.load("shannon.pkl")
    #利用模型得到不同的预测值
    y_pred=model.predict(x)
    #得到R^2值
    score=sklearn.metrics.r2_score(y, y_pred)
    #画图，具体说明看函数
    draw(y,y_pred,score,'Shannon','#00CED1')
