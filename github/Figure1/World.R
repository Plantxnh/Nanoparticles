...
library(mapdata)
mp<-NULL #定义一个空的地图
mapworld<-borders("world",colour = "gray",fill="white") #绘制基本地图
mp<-ggplot()+mapworld+ylim(-60,90)
mp#利用ggplot呈现，同时地图纵坐标范围从-60到90
mp2<-mp+geom_point(aes(x=visit.x,y=visit.y,size=mydata$number),color="#66C2A5",alpha=0.8)+scale_size(range=c(6,15))
