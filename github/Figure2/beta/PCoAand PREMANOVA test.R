...
#或者现有的距离矩阵,就是之前算好的或者公司给的用于重新分组做pcoa，这边不做演示
#dis <- read.delim('group(Archaea).txt', row.names = 1, sep = '\t', stringsAsFactors = FALSE, check.names = FALSE)
...
#排序（基于 OTU 丰度表）
distance <- vegdist(otu, method = 'euclidean') #这边方法与上面pcoa一样，用的BC，这个代码是vegan包的函数，所以不能计算unifrac距离，因此才会有下文基于现有距离矩阵计算pcoa，也就是说你要做UNifrac距离的pcoa图，得先自己算好距离，再把距离矩阵输入到下面基于距离矩阵的代码
...
pcoa_plot <- ggplot(sample_site, aes(PCoA1, PCoA2, group = genotype)) +
  ...
  geom_point(aes(color =genotype, shape =genotype), size = 6, alpha = 0.8) + #可在这里修改点的透明度、大小
  scale_shape_manual(values = c(17,19,19,19,19,19,19,15,15,15,15,15,15)) + #可在这里修改点的形状
  scale_color_manual(values = c('#7FC97F', '#BEAED4','#FDC086','#F0027F','#FFFF99','#386CB0','#BF5B17','#BEAED4','#FDC086','#F0027F','#FFFF99','#386CB0','#BF5B17')) + #可在这里修改点的颜色，以此对应过去的
...
#adonis
#如果最开始输入的是otu表格的话
adonis_result_otu <- adonis(otu~genotype, group, permutations = 999, distance = 'bray')  #这边方法与上面pcoa一样，用的BC
#查看数值
